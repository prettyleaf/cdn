...
- `<provider>_domains.txt` - optional external domain list (kept separate from IP ranges).
...
- `<provider>_mihomo.mrs` – binary [Mihomo rule-provider](https://wiki.metacubex.one/en/config/rule-providers/#format) format for IP CIDR (IPv4 + IPv6).
- `<provider>_mihomo_ipv4.mrs` – the same, but IPv4-only.
- `<provider>_mihomo_domains.mrs` - binary Mihomo rule-provider format for domains (generated only when `<provider>_domains.txt` exists and is not empty).

Need every provider in a single rule set? Use the `all/` directory, which aggregates every IP prefix before generating the same IP-based files. Domain lists are not merged into `all/*`.
